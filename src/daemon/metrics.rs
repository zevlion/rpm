...
use crate::daemon::manager::ProcessMap;
...
pub fn start(map: ProcessMap) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            // First sample — seeds the CPU baseline
            sys.refresh_processes_specifics(ProcessesToUpdate::Some(&sys_pids), true, refresh_kind);

            // Short pause so sysinfo can compute a meaningful CPU delta
            time::sleep(Duration::from_millis(200)).await;

            // Second sample — CPU % is now accurate
            sys.refresh_processes_specifics(ProcessesToUpdate::Some(&sys_pids), true, refresh_kind);

            // Write stats back into the map
